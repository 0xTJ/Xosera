...
XOSERA_CLEAN := 1
$(info === Xosera XoseRAM [$(XOSERA_HASH)] is CLEAN from git)
else
# "dirty" (HDL modified) from git
XOSERA_CLEAN := 0
$(info === Xosera XoseRAM [$(XOSERA_HASH)] is DIRTY: $(DIRTYFILES))
endif
...
SRC := xoseram/$(TOP).sv $(wildcard $(SRCDIR)/*.sv)
...
VLT_CONFIG := xoseram/ice40_config.vlt
...
all: xoseram/$(OUTNAME).bin $(MAKEFILE_LIST)
	@echo === Finished Building XoseRAM Xosera ===
.PHONY: all

# program XoseRAM FPGA via USB (may need udev rules or sudo on Linux)
prog: xoseram/$(OUTNAME).bin $(MAKEFILE_LIST)
	@echo === Programming XoseRAM Xosera ===
	$(ICEPROG) -d i:0x0403:0x6014 $(TOP).bin
.PHONY: prog

# run icetime to generate a timing report
timing: xoseram/$(OUTNAME).rpt $(MAKEFILE_LIST)
...
$(DOT): %.dot: %.sv $(MAKEFILE_LIST)
	mkdir -p xoseram/dot $(@D)
	$(YOSYS) $(YOSYS_ARGS) -l $(LOGS)/$(OUTNAME)_yosys.log -q -p 'verilog_defines $(DEFINES) -DSHOW ; read_verilog -I$(SRCDIR) -sv $< ; show -enum -stretch -signed -width -prefix xoseram/dot/$(basename $(notdir $<)) $(basename $(notdir $<))'

# synthesize Verilog and create json description
%.json: $(VLT_CONFIG) $(RESET_COPMEM) $(SRC) $(INC) $(FONTFILES) $(MAKEFILE_LIST)
	@echo === Building XoseRAM Xosera ===
	@rm -f $@
	@mkdir -p $(LOGS) $(@D)
...
xoseram/%_$(OUTSUFFIX).asc: xoseram/%_$(OUTSUFFIX).json $(PIN_DEF) $(MAKEFILE_LIST)
	@-rm -f $@
	@mkdir -p $(LOGS) $(@D)
...
ifdef NO_PNR_RETRY
	...
	@echo === XoseRAM Xosera: $(VIDEO_MODE) | tee $(OUTNAME)_stats.txt
...
xoseram/%_$(OUTSUFFIX).bin: xoseram/%_$(OUTSUFFIX).asc $(MAKEFILE_LIST)
	@rm -f $@
	$(ICEPACK) $< $@
	$(ICEMULTI) -v -v -p0 xoseram/*.bin -o $(TOP).bin

# make timing report from ASCII bitstream
xoseram/%_$(OUTSUFFIX).rpt: xoseram/%_$(OUTSUFFIX).asc $(MAKEFILE_LIST)
	@rm -f $@
...
clean:
	rm -f $(VLT_CONFIG) xosera_xoseram.bin $(wildcard xoseram/*.json) $(wildcard xoseram/*.asc) $(wildcard xoseram/*.rpt) $(wildcard xoseram/*.bin)
.PHONY: clean
